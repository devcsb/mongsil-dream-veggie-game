...
assert(gameSrc.includes('function drawKey'), 'game has dedicated key renderer');
assert(gameSrc.includes("v.type === 'key'"), 'drawVegetable branches on key type');
assert(gameSrc.includes('homeY'), 'fallblocks store homeY for respawn reset');
const stagesSrc = fs.readFileSync(path.join(root, 'stages.js'), 'utf8');
assert(stagesSrc.includes('scatterConceptDecor'), 'stages use concept-specific decor scatter');
